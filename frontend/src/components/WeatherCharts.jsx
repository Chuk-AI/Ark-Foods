...
const WeatherCharts = () => {
  ...
  const [selectedChart, setSelectedChart] = useState("precipitation"); // Default chart
...
useEffect(() => {
  ...
  const renderCharts = (data) => {
    ...
    // Validate data structure before attempting to render charts
    if (!data || !data.forecast_data) {
      console.error("Invalid data passed to renderCharts.");
      return;
    }
    ...
    // Render the selected chart based on the selectedChart state
    switch (selectedChart) {
      case "precipitation":
        if (data.forecast_data.PRECIP && data.forecast_data.PRECIP.dates?.length) {
          ...
          console.warn("No precipitation data available to render the chart.");
        }
        break;
      ...
      case "temperature":
        if (data.forecast_data.TAVG && data.forecast_data.TAVG.dates?.length) {
          ...
          console.warn("No temperature data available to render the chart.");
        }
        break;
  
      case "ensemble":
        if (data.accumulation_data?.ensemble_totals?.length) {
          const sortedEnsembleTotals = data.accumulation_data.ensemble_totals;
          const climatologyValue = data.accumulation_data.accumulated_climo_precip || 0;
          createBarChart(ensembleChartRef, sortedEnsembleTotals, climatologyValue);
        } else {
          console.warn("No ensemble analysis data available to render the chart.");
        }
        break;
  
      default:
        console.error(`Unknown selectedChart value: ${selectedChart}`);
    }
  };
  ...
  <ul>
    <li
      className={selectedChart === "precipitation" ? "weather-sidebar-active" : ""}
      onClick={() => setSelectedChart("precipitation")}
    >
      Precipitation Chart
    </li>
    <li
      className={selectedChart === "temperature" ? "weather-sidebar-active" : ""}
      onClick={() => setSelectedChart("temperature")}
    >
      Temperature Chart
    </li>
    <li
      className={selectedChart === "ensemble" ? "weather-sidebar-active" : ""}
      onClick={() => setSelectedChart("ensemble")}
    >
      Ensemble Analysis Chart
    </li>
  </ul>
</div>
