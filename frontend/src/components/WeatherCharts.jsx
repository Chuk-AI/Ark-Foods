...
const WeatherCharts = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`/api/weather_forecasts?lat=${city.split(',')[0]}&lon=${city.split(',')[1]}&start=${startDate}&end=${endDate}`);
      ...
      const formattedClimatology = formatClimatologyData(convertedData.daily_climatology, convertedData.forecast_data.TAVG.dates);
  ...
  const convertTemperatureData = (data, unit) => {
    ...
    if (data.daily_climatology.TAVG) {
      data.daily_climatology.TAVG = Object.fromEntries(Object.entries(data.daily_climatology.TAVG).map(([key, value]) => [key, convert(value)]));
    }
    ...
    <div className="weather-layout">
      <div className="weather-inputs">
        <input
          ...
          style={{ margin: '20px', border: '1px solid #33b2a8', padding: '7px', borderRadius: '10px' }}
        />
        <input
          type="date"
          value={endDate}
