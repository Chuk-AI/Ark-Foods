...
const TerminalViolinPlot = () => {
  ...
  // Fetch data with useCallback for optimization
  const fetchTerminalViolinData = useCallback(async () => {
  ...
  return (
    <div id="terminal-violin-plot-section" className="section violin-chart-container">
      ...
      >
        {/* Show loading state inside the individual chart containers */}
        {loading ? (
          <>
            <div style={chartContainerStyle}>
              <h3 style={{ textAlign: "center" }}>Loading USDA Data...</h3>
            </div>
            <div style={chartContainerStyle}>
              <h3 style={{ textAlign: "center" }}>Loading ProduceIQ Data...</h3>
            </div>
          </>
        ) : (
          Object.entries(terminalViolinData).map(([source, data]) => (
            <div key={source} style={chartContainerStyle}>
              <Plot
                data={[
                  {
                    type: "violin",
                    x: data.map((item) => item.varietyName),
                    y: data.map((item) => item.price),
                    line: { color: source === "USDA" ? "#1f77b4" : "#ff7f0e" },
                    box: { visible: true },
                    meanline: { visible: true },
                  },
                ]}
                layout={{
                  title: {
                    text: `${source} Terminal Data`,
                    font: { size: 16, weight: "bold" },
                  },
                  xaxis: {
                    title: { text: "Variety", font: { size: 14, weight: "bold" } },
                    automargin: true,
                  },
                  yaxis: {
                    title: { text: "Avg Daily Price", font: { size: 14, weight: "bold" } },
                    automargin: true,
                  },
                  height: 500,
                  width: 600,
